...
const BASE_FULL_PATH_MODULE = path.resolve(
...
function cleanPackEntry(data, { clearSourceId = true, ownership = 0 } = {}) {
    ...
    if (data._stats?.lastModifiedBy)
        data._stats.lastModifiedBy = "packsbuilder0000";
    ...
    if (
        (data.system?.weight || data.system?.weight === 0) &&
        !data.system?.weight?.units
    ) {
        data.system.weight = {
            value:
                typeof data.system.weight === "number" ? data.system.weight : 0,
            units: "lb",
        };
    }
    ...
    if (data.system?.description?.value)
        data.system.description.value = cleanString(
 ...
 *
 * - `vp run build:clean` - Clean all source JSON files.
 * - `vp run build:clean -- classes` - Only clean the source files for the specified compendium.
 * - `vp run build:clean -- classes Barbarian` - Only clean a single item from the specified compendium.
 */
 ...
 *
 * - `vp run build:db` - Compile all JSON files into their LevelDB files.
 * - `vp run build:db -- classes` - Only compile the specified pack.
 */
...
async function extractPacks(packName, entryName) {
    ...
    const module = JSON.parse(
        fs.readFileSync(MODULE_JSON_FULL_PATH, { encoding: "utf8" }),
    ); // MOD 4535992 /src/module.json instead ./system.json and rename system to module
    ...
    for (const packInfo of packs) {
        ...
        logger.info(
            `Extracting pack ${packInfo.name} from ${packInfoPath} to ${dest}`,
        );
