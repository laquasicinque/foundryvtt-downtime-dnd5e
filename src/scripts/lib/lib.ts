...
export async function runMacroOnExplicitActor(explicitActor, macroReference, ...macroData) {
    ...
    */
    ...
    try {
        ...
        macroTmp.ownership.default = CONST.DOCUMENT_OWNERSHIP_LEVELS.OWNER;
...
export function getOwnedCharacters(user?: User.Stored) {
    user = user || game.user;
    return game.actors
        .filter((actor) => {
            return (
                actor.ownership?.[user.id] === CONST.DOCUMENT_OWNERSHIP_LEVELS.OWNER && actor.prototypeToken.actorLink
            );
        })
        .sort((a, b) => {
