...
export class RetrieveHelpers {
  ...
  static retrieveUuid({
    ...
    if (documentCollectionType || pack === "world") {
      ...
      if (!collection) {
        ...
        const doc =
          collection.find(
            (e) => e.id === documentId || e.name === documentName,
          ) || null;
        if (doc) {
          return doc.uuid;
        }
    ...
    if (documentPack) {
      const pack = RetrieveHelpers.getCompendiumCollectionSync(
        documentPack,
        ignoreError,
      );
      if (!pack) {
        ...
        const doc =
          pack.index.find(
            (i) => i._id === documentId || i.name === documentName,
          ) || null;
        if (doc) {
          return doc.uuid;
        }
  ...
  static getCompendiumCollectionSync(
    target,
    ignoreError = false,
    ignoreName = true,
  ) {
    ...
    if (!targetTmp) {
      if (ignoreError) {
        ...
        throw Logger.error(
          `CompendiumCollection is not found`,
          true,
          targetTmp,
        );
      }
  ...
  static async getCompendiumCollectionAsync(
    target,
    ignoreError = false,
    ignoreName = true,
  ) {
  ...
  static async getJournalAsync(target, ignoreError = false, ignoreName = true) {
    ...
    // Type checking
    if (!(targetTmp instanceof foundry.documents.collections.Journal)) {
      if (ignoreError) {
        Logger.warn(`Invalid Journal`, false, targetTmp);
        return;
      } else {
  ...
  static getPlaylistSoundPathSync(
    target,
    ignoreError = false,
    ignoreName = true,
  ) {
    ...
    if (RetrieveHelpers.stringIsUuid(targetTmp)) {
      ...
      targetTmp = game.playlists.contents
        ...
        .find((playlistSound) => {
          return (
            playlistSound.id === targetTmp || playlistSound.name === targetTmp
          );
        });
    }
    if (!targetTmp) {
  ...
  static async getPlaylistSoundPathAsync(
    target,
    ignoreError = false,
    ignoreName = true,
  ) {
  ...
  static async getRollTableAsync(
    target,
    ignoreError = false,
    ignoreName = true,
  ) {
