...
type OnlySelectedPartial<T, K extends keyof T> = T extends unknown ? Prettify<
    Partial<Pick<T, K>> & Required<Omit<T, K>>> : never

export function createTrackedItem<T extends Downtime.TrackedItem>(options: OnlySelectedPartial<T, UnionSameKeys<T>>): T {
    return {
