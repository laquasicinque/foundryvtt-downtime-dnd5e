...
export default class TrackingAndTraining {
    ...
    static async editCategory(actorId: string, categoryId: string, world = false) {
        ...
        const thisCategory = allCategories.find((obj) => obj.id === categoryId);
        new CategoryApp({
            actor,
    ...
    static async deleteCategory(actorId: string, categoryId: string, world = false) {
        ...
        if (del) {
            ...
            if (world) {
                ...
                await setActorCategories(actor, allCategories.filter(x => x.id !== categoryId))
    ...
    static async editFromSheet(actorId: string, itemId: string, dropdownOptions: Downtime.DropdownOptions, world = false) {
    ...
    static async deleteFromSheet(actorId: string, itemId: string, world = false) {
        ...
        if (del) {
            ...
            if (world) {
                ...
                await setActorActivities(actor, items.filter(item => activity.id !== item.id))
    ...
    static getRollResult(roll: Downtime.Roll[]) {
        return sum([...pluck(roll, 'total')].filter((x): x is number => x != null))
    }
    ...
    static async checkCompletion(actor: dnd5e.documents.Actor5e, activity: Downtime.TrackedItem, alreadyCompleted: boolean) {
        if (alreadyCompleted) {
            return;
        }
    ...
    static async importItems(actorId: string) {
        ...
        input.addEventListener("change", function (event) {
            ...
            foundry.utils.readTextFromFile(file).then(async (contents) => {
                ...
                const currentCategoryNames = currentCategories.map((c) => c.name);
                ...
                // let act = "quit";
                //let content = `<p><b>${localize("downtime-dnd5e.ImportTypeSelectionOverwrite")}:</b> ${localize("downtime-dnd5e.ImportTypeSelectionTextOverwrite")}</p>
                //               <p><b>${localize("downtime-dnd5e.ImportTypeSelectionCombine")}:</b> ${localize("downtime-dnd5e.ImportTypeSelectionTextCombine")}</p>`;
                // Create dialog
                // new Dialog({
                //   title: localize("downtime-dnd5e.ImportTypeSelectionTitle"),
                //   content: content,
                //   buttons: {
                //     overwrite: {icon: "<i class='fas fa-file-import'></i>", label: localize("downtime-dnd5e.ImportTypeSelectionOverwrite"), callback: () => act = "overwrite"},
                //     add: {icon: "<i class='fas fa-plus'></i>", label: localize("downtime-dnd5e.ImportTypeSelectionCombine"), callback: () => act = "add"},
                //     quit: {icon: "<i class='fas fa-times'></i>", label: localize("downtime-dnd5e.Cancel"), callback: () => act = "quit"},
                //   },
                //   default: "quit",
                //   close: async (html) => {
                //     if(act === "quit"){
                ...
                //       if(possibleDupes){
                //         new Dialog({
                //           title: localize("downtime-dnd5e.ImportDupeWarningTitle"),
                //           content: `<p>${localize("downtime-dnd5e.ImportDupeWarningText")}</p>`,
                //           buttons: {
                //             ok: {icon: "<i class='fas fa-check'></i>", label: localize("downtime-dnd5e.ImportDupeWarningConfirm")},
                //           },
                //           default: "ok"
                //         }).render({ force: true });
                //       }
