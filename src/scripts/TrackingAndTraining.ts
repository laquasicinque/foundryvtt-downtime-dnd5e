...
export const TrackingAndTraining = {
  ...
  async deleteCategory(actorId: string, categoryId: string, world = false) {
    ...
    if (del) {
      ...
      // Update actor
      await setCategories({
        actor: world ? null : actor,
        categories: allCategories.filter((x) => x.id !== categoryId),
      });
  ...
  async updateItemProgressFromSheet(
    actorId: string,
    itemId: string,
    value: string,
    world = false,
  ...
  async progressItem(actorId: string, itemId: string, world = false) {
    ...
    if (rollType === "ABILITY") {
      ...
      const r = await actor.rollAbilityCheck({
        ...options,
  ...
  determineRollType(item: Downtime.TrackedItem) {
    ...
    if (
      ["ABILITY", "SKILL", "TOOL", "MACRO", "FIXED"].includes(
        item.progressionStyle,
      )
    ) {
  ...
  getActorTools(actorId: string) {
    const actor = getActor(actorId);
    const items = actor.items as foundry.abstract.EmbeddedCollection<
      dnd5e.documents.Item5e,
  ...
  async importItems(actorId: string) {
    ...
    input.addEventListener("change", async function (event) {
      ...
      const contents = await foundry.utils.readTextFromFile(file);
      const importedData = JSON.parse(contents);
      let importedItems: Downtime.TrackedItem[] = [];
      let importedCategories: Downtime.Category[] = [];
      ...
      if (importedItems.length < 1 && importedCategories.length < 1) {
      ...
      for (const category of importedCategories) {
        ...
        if (currentCategoryIds.has(category.id)) {
          ...
          importedItemsByCategory
            .get(oldId)
            ?.forEach((item) => (item.category = newId));
          importedItemsByCategory.set(
            newId,
            importedItemsByCategory.get(oldId)!,
          );
          category.id = newId;
        }
        ...
        if (currentCategoryNames.has(category.name)) {
          let existingCategory = currentCategories.find(
            (obj) => obj.name === category.name,
          )!;
          let existingCategoryId = existingCategory.id;
          let importedCategoryId = category.id;

          importedItemsByCategory
            .get(importedCategoryId)
            ?.forEach((item) => (item.category = existingCategoryId));

          // Flag these categories for deletion
          categoriesToDelete.add(importedCategoryId);
        }
      }

      const currentItems = getActorActivities(actor);
      const currentIds = new Set(pluckId(currentItems));
      const availableCategoryIds = new Set([
        ...currentCategoryIds,
        ...pluckId(importedCategories),
      ]);
      ...
      for (const item of importedItems) {
        ...
        if (
          ...
          !availableCategoryIds.has(item.category)
        ) {
          item.category = "";
        }
      }
      ...
      const combinedCategories = [
        ...currentCategories,
        ...importedCategories,
      ].filter((c) => !categoriesToDelete.has(c.id));
  ...
  async moveActivity({
    ...
    if (source.category === target.category) {
      ...
      if (sourceActor === targetActor) {
        ...
        const newSourceActivities = sourceActivities.filter(
