<script lang="ts">
  import ActorActivityButtons from "../../../partial/ActorActivityButtons.svelte";
  ...
  import WorldActivityButtons from "../../../partial/WorldActivityButtons.svelte";
  ...
  const {
    actor,
    sheet: sheetRaw,
  }: { sheet: any; actor: dnd5e.documents.Actor5e<"character"> } = $props();
  ...
  const addItemInCategory = pubs(
    async (category: string) =>
      await TrackingAndTraining.addItem(actor.id!, dropdownOptions, false, {
        category,
      })
  );
  const addItemInWorldCategory = pubs(
    async (category: string) =>
