...
import ActorActivityButtons from "./ActorActivityButtons.vue";
...
const categorizedActorActivities = computed(() =>
    categoriesActor
        .map(
            (category) =>
                ({
...
const uncategorizedActorActivities = computed(
    () =>
        ({
            id: "",
            description: "",
...
const categorizedWorldActivities = computed(() =>
    categoriesWorld
        .map(
            (category) =>
                ({
...
const uncategorizedWorldActivities = computed(
    () =>
        ({
            id: "",
            description: "",
...
const hasCharacterDowntimes = computed(
    () =>
        !!uncategorizedActorActivities.value.activities.length ||
        categorizedActorActivities.value.some((x) => x.activities.length),
);
const hasWorldDowntimes = computed(
    () =>
        !!uncategorizedWorldActivities.value.activities.length ||
        categorizedWorldActivities.value.some((x) => x.activities.length),
);

const editCategory = async (category: string) => await TrackingAndTraining.editCategory(sheet.actor.id!, category);
const deleteCategory = async (category: string) => await TrackingAndTraining.deleteCategory(sheet.actor.id!, category);
...
<template>
    <ActorActivityButtons v-if="!showToUserEditMode" :show-import-button="showImportButton">
        <h4>Character Downtimes</h4>
    </ActorActivityButtons>
    <section class="items-list downtime-list">
        <ActivitiesTable
            v-if="uncategorizedActorActivities.activities.length"
            :category="uncategorizedActorActivities"
            @edit-category="(catId) => editCategory(catId)"
            @delete-category="(catId) => deleteCategory(catId)"
            @edit-activity="(itemId) => editActivity(itemId)"
            @delete-activity="(itemId) => deleteActivity(itemId)"
            @roll-activity="(itemId) => rollActivity(itemId)"
            @set-progress="(payload) => setProgress(payload)"
            @move-activity-up="(itemId) => moveActivityUp(itemId)"
            @move-activity-down="(itemId) => moveActivityDown(itemId)"
        />
        <ActivitiesTable
            v-for="category of categorizedActorActivities"
            :key="category.id"
            :category="category"
            @edit-category="(catId) => editCategory(catId)"
            @delete-category="(catId) => deleteCategory(catId)"
    ...
    <section class="items-list downtime-list">
        <ActivitiesTable
            v-if="uncategorizedWorldActivities.activities.length"
            :category="uncategorizedWorldActivities"
            @edit-category="(catId) => editWorldCategory(catId)"
            @delete-category="(catId) => deleteWorldCategory(catId)"
            @edit-activity="(itemId) => editWorldActivity(itemId)"
            @delete-activity="(itemId) => deleteWorldActivity(itemId)"
            @roll-activity="(itemId) => rollWorldActivity(itemId)"
            @set-progress="(payload) => setWorldProgress(payload)"
            @move-activity-up="(itemId) => moveWorldActivityUp(itemId)"
            @move-activity-down="(itemId) => moveWorldActivityDown(itemId)"
        />
        <ActivitiesTable
            v-for="category of categorizedWorldActivities"
            :key="category.id"
            :category="category"
            @edit-category="(catId) => editWorldCategory(catId)"
            @delete-category="(catId) => deleteWorldCategory(catId)"
