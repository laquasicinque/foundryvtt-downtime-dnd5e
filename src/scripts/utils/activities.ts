...
export function getActorActivities(actorOrId: string | dnd5e.documents.Actor5e) {
  return getActor(actorOrId).getFlag(CONSTANTS.MODULE_ID, CONSTANTS.FLAGS.trainingItems) ?? [];
}

export function getActorActivitiesMap(actorOrId: string | dnd5e.documents.Actor5e) {
...
export function setActorActivities(
  actorOrId: string | dnd5e.documents.Actor5e | foundry.documents.Actor.Stored,
