...
const API = {
  ...
  async updateActivity(
    ...
    if (itemIdx < 0) {
      Logger.warn(
        localize("downtime-dnd5e.ItemNotFoundWarning") + ": " + activityName,
        true,
      );
      return;
    }
  ...
  getCategories(actor: Actor | string) {
    const actorTmp = RetrieveHelpers.getActorSync(actor, false, false);
    if (actorTmp) return getActorCategories(actorTmp);
    Logger.warn(localize("downtime-dnd5e.ActorNotFoundWarning"), true);
    return [];
  },
   ...
   * @returns {Promise<void>}
   */
  async updateWorldActivityProgress(
    activityName: string,
    newProgress: number,
    explicitActor: Actor,
  ) {
    return await this.updateWorldActivity(
      activityName,
      newProgress,
      explicitActor,
    );
