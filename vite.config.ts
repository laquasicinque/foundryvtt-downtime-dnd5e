import cleanPlugin from "vite-plugin-clean";
import { defineConfig, normalizePath, lazyPlugins } from "vite-plus";
import type { PluginOption } from "vite-plus";
import path from "path";
import { fileURLToPath } from "url";
import { svelte } from "@sveltejs/vite-plugin-svelte";
import { viteStaticCopy } from "vite-plugin-static-copy";

// For convenience, you just need to modify the package ID below as it is used to fill in default proxy settings for
// the dev server.
const s_MODULE_ID = "downtime-dnd5e";
const s_PACKAGE_ID = "modules/" + s_MODULE_ID;
const s_ENTRY_JAVASCRIPT = "module.ts";

const s_COMPRESS = false; // Set to true to compress the module bundle.
const s_SOURCEMAPS = true; // Generate sourcemaps for the bundle (recommended).
const configDir = path.dirname(fileURLToPath(import.meta.url));

// ATTENTION!
// You must change `base` and the `proxy` strings replacing `/modules/${s_MODULE_ID}/` with your
// module or system ID.

export default defineConfig({
    staged: {
        "*": "vp check --fix",
    },
    lint: {
        jsPlugins: [
            { name: "vite-plus", specifier: "vite-plus/oxlint-plugin" },
        ],
        rules: { "vite-plus/prefer-vite-plus-imports": "error" },
        options: { typeAware: true, typeCheck: true },
    },
    fmt: {
        printWidth: 80,
        tabWidth: 4,
        sortPackageJson: false,
        svelte: {},
        ignorePatterns: [
            "/dist",
            "/package-lock.json",
            "/.pnp.js",
            "/.yarn/",
            "/.vscode/",
            "/.idea",
            ".vite-cache/**",
            "**/*.md",
            "/src/packs",
        ],
    },
    root: "src/", // Source location / esbuild root.
    base: `/${s_PACKAGE_ID}/`, // Base module path that 30001 / served dev directory.
    publicDir: false as const, // No public resources to copy.
    cacheDir: "../.vite-cache", // Relative from root directory.

    resolve: { conditions: ["import", "browser"] },

    // About server options:
    // - Set to `open` to boolean `false` to not open a browser window automatically. This is useful if you set up a
    // debugger instance in your IDE and launch it with the URL: 'http://localhost:30001/game'.
    //
    // - The top proxy entry redirects requests under the module path for `style.css` and following standard static
    // directories: `assets`, `lang`, and `packs` and will pull those resources from the main Foundry / 30000 server.
    // This is necessary to reference the dev resources as the root is `/src` and there is no public / static
    // resources served with this particular Vite configuration. Modify the proxy rule as necessary for your
    // static resources / project.
    server: {
        port: 29999,
        open: "/game",
        strictPort: true, // Prevents switching to a different port if 29999 is unavailable
        // open: false,
        watch: {
            usePolling: true, // Ensures file changes are detected
        },
        proxy: {
            // Serves static files from main Foundry server.
            [`^(/${s_PACKAGE_ID}/(images|fonts|assets|lang|languages|packs|styles|templates|style.css))`]:
                "http://127.0.0.1:30000",

            // All other paths besides package ID path are served from main Foundry server.
            [`^(?!/${s_PACKAGE_ID}/)`]: "http://127.0.0.1:30000",

            // Enable socket.io from main Foundry server.
            "/socket.io": { target: "ws://127.0.0.1:30000", ws: true },
        },
    },

    build: {
        outDir: normalizePath(path.resolve(configDir, `./dist/${s_MODULE_ID}`)),
        emptyOutDir: false,
        sourcemap: s_SOURCEMAPS,
        minify: s_COMPRESS ? ("terser" as const) : false,
        target: ["es2022", "chrome100"],
        lib: {
            entry: path.resolve(configDir, "./src/" + s_ENTRY_JAVASCRIPT), // "./module.svelte.ts"
            formats: ["es" as const],
            fileName: "module",
        },
        rollupOptions: {
            external: ["pixi.js", "tidy5e-sheet"], // Prevent bundling PIXI.js
        },
    },

    plugins: lazyPlugins(() => [
        viteStaticCopy({
            targets: [
                //   {
                //     src: normalizePath(path.resolve(configDir, './src/assets')) + '/[!.]*', // 1️
                //     dest: normalizePath(path.resolve(configDir, `./dist/${s_MODULE_ID}/assets`)), // 2️
                //   },
                //   {
                //     src: normalizePath(path.resolve(configDir, './src/images')) + '/[!.]*', // 1️
                //     dest: normalizePath(path.resolve(configDir, `./dist/${s_MODULE_ID}/images`)), // 2️
                //   },
                //   {
                //     src: normalizePath(path.resolve(configDir, './src/icons')) + '/[!.]*', // 1️
                //     dest: normalizePath(path.resolve(configDir, `./dist/${s_MODULE_ID}/icons`)), // 2️
                //   },
                {
                    src: normalizePath(
                        path.resolve(configDir, "./src/templates/*.hbs"),
                    ), // 1️
                    dest: normalizePath(
                        path.resolve(
                            configDir,
                            `./dist/${s_MODULE_ID}/templates`,
                        ),
                    ), // 2️
                },
                //   {
                //     src: normalizePath(path.resolve(configDir, './src/lang')) + '/[!.]*',
                //     dest: normalizePath(path.resolve(configDir, `./dist/${s_MODULE_ID}/lang`)),
                //   },
                {
                    src: normalizePath(
                        path.resolve(configDir, "./src/languages/*.json"),
                    ),
                    dest: normalizePath(
                        path.resolve(
                            configDir,
                            `./dist/${s_MODULE_ID}/languages`,
                        ),
                    ),
                },
                //   {
                //     src: normalizePath(path.resolve(configDir, './src/styles')) + '/**/*.css',
                //     dest: normalizePath(path.resolve(configDir, `./dist/${s_MODULE_ID}/styles`)),
                //   },
                //   {
                //     src: normalizePath(path.resolve(configDir, './src/packs')) + '/[!.^(_?.*)]*', // + '/[!.^(_source)]*',
                //     dest: normalizePath(path.resolve(configDir, `./dist/${s_MODULE_ID}/packs`)),
                //   },
                {
                    src: normalizePath(
                        path.resolve(configDir, "./src/module.json"),
                    ),
                    dest: normalizePath(
                        path.resolve(configDir, `./dist/${s_MODULE_ID}/`),
                    ),
                },
                //       {
                //     src: normalizePath(path.resolve(configDir, './src/scripts/libs')) + '/[!.]*',
                //     dest: normalizePath(path.resolve(configDir, `./dist/${s_MODULE_ID}/scripts/libs`)),
                //   },
            ],
        }) as PluginOption,
        svelte({
            configFile: path.resolve(configDir, "svelte.config.js"),
        }) as PluginOption,
        cleanPlugin() as unknown as PluginOption,
    ]),
});
